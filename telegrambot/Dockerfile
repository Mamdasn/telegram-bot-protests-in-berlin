...
CMD ["sh", "-c", "/app/initial-setup-files/webhook-setup.sh && /app/initial-setup-files/nginx-setup.sh && nginx && gunicorn --bind 0.0.0.0:5000 wsgi:app >> /app/output/telegram.logs 2>&1"]
