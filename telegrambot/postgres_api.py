...
class Fetchpostgres:
    ...
    def get_query_column(self, column, query):
        ...
        with self.connection(**self.params).cursor() as cursor:
            ...
            cursor.execute("SELECT * FROM events WHERE " + query_statement, ([f"%{q}%" for q in query],))
            fetched_records = cursor.fetchall()
            return fetched_records
    def get_query_any_column(self, query, columns=['Aufzugsstrecke', 'Versammlungsort', 'Thema', 'PLZ']):
        """
        ...
        with self.connection(**self.params).cursor() as cursor:
            ...
            if 'Datum' in columns: 
                query_statement = query_statement.replace('Datum', "TO_CHAR(Datum, 'DD.MM.YYYY')")
            query_statement = ' AND '.join([f"({query_statement})" for q in query])
            query_condition = [f"%{q}%" for q in query for c in columns]
            today_date = datetime.datetime.today().strftime("%Y.%m.%d")
            cursor.execute(f"SELECT DISTINCT ON ({', '.join(columns)}) * FROM events WHERE ((Datum >= '{today_date}') AND {query_statement})", query_condition)
            fetched_records = cursor.fetchall()
    ...
    def format_postgres_output(self, q):
        ...
        time_range = f'{q[2].strftime("%H:%M")} to {q[3].strftime("%H:%M:")}' if q[3] else f'{q[2].strftime("%H:%M")}' if q[2] else ''
        ...
        versammlungsort = f'<b>Versammlungsort</b>: <a href="{google_maps_url}">{q[6]}{newline}</a>' if q[6] else ''
        route_with_google_maps_urls = ''
        if q[7]:
            find_indexes = [q[7].find(sep) for sep in [' - ', '/'] if q[7].find(sep)!=-1]
            if find_indexes:
        ...
        aufzugsstrecke = f'<b>Aufzugsstrecke</b>: {route_with_google_maps_urls}{newline}' if q[7] else ''
...
#
#from telegram_bot_api import message_format_for_postgres
#import datetime
#
#today = datetime.datetime.today().strftime('%Y.%m.%d')
#queries = [fetcher.format_postgres_output(q) for q in fetcher.getBySpecificDate(today)]
#page = message_format_for_postgres(queries, page_number=1)
#print(page)

# queries = [
#         #    fetcher.getBySpecificDate('2023.02.27'), 
#         #    fetcher.getThemaQuery(['Kundgebung zum Tag der Arbeit', ]),
#         #    fetcher.getBisQuery('14:00'),
#         #    fetcher.getVonQuery('14:00'),
#         #    fetcher.getVersammlungsortQuery(['Unter',  'den Linden', '63'],operator=['and', 'and']),
#            fetcher.getAufzugsstreckeQuery(['Hermannplatz', 'unter den Linden'], operator='or'),
#            ]
# for q in queries:
#     for qq in q:
#         print(qq)
